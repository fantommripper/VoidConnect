...
impl ProfilePage {
    ...
    fn show_inner(&mut self, ui: &mut egui::Ui) {
        ...
        match self.port_reachable {
            ...
            Some(false) => {
                ui.label(
                    egui::RichText::new("󰀦  Прямой доступ извне не подтверждён")
                        .strong()
                ...
                ui.label(
                    egui::RichText::new(
                        "Bootstrap-узел не смог подключиться к вам напрямую (провайдер/файрвол/\
                         symmetric NAT). Прямая раздача файлов и входящие подключения могут не \
                         работать; личные сообщения и файлы пойдут через relay. Проба проверяет \
                         совпадение внешнего и внутреннего порта — при symmetric NAT возможны \
                         ложные срабатывания, даже если UPnP-проброс реально работает. Чтобы \
                         гарантировать прямой доступ, откройте порты на роутере/файрволе \
                ...
                if self.bootstrap {
                    ...
                    ui.label(
                        egui::RichText::new(
                            "\u{F0026} Если прямой доступ действительно закрыт, режим Bootstrap-узла \
                             работать не будет: другие узлы не смогут к вам подключиться. Проверьте \
